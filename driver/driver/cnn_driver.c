...
ssize_t cnn_read(struct file *pfile, char __user *buffer, size_t length, loff_t *offset) {
	...
	switch (minor) {
	case 0:
		printk(KERN_INFO "cnn_read Succesfully read from CNN device 0.\n");
		ready = ioread32(tp->base_addr + XIL_CNN_READY_OFFSET);
		if (ready) {
			for (int i = 0; i < 10; i++) {
				r[i] = ioread32(tp->base_addr + XIL_CNN_R_0_OFFSET + 4*i);
			}
			//r[0] = ioread32(tp->base_addr + XIL_CNN_R_0_OFFSET);
			//r[1] = ioread32(tp->base_addr + XIL_CNN_R_1_OFFSET);
			//r[2] = ioread32(tp->base_addr + XIL_CNN_R_2_OFFSET);
			//r[3] = ioread32(tp->base_addr + XIL_CNN_R_3_OFFSET);
			//r[4] = ioread32(tp->base_addr + XIL_CNN_R_4_OFFSET);
			//r[5] = ioread32(tp->base_addr + XIL_CNN_R_5_OFFSET);
			//r[6] = ioread32(tp->base_addr + XIL_CNN_R_6_OFFSET);
			//r[7] = ioread32(tp->base_addr + XIL_CNN_R_7_OFFSET);
			//r[8] = ioread32(tp->base_addr + XIL_CNN_R_8_OFFSET);
			//r[9] = ioread32(tp->base_addr + XIL_CNN_R_9_OFFSET);
			
			//if(ready) {
			//	for (int i = 0; i < 10; i++) {
			//		scnprintf(buf, BUFF_SIZE, "%d ", r[i]);
			//	}
			//	scnprintf(buf, BUFF_SIZE, "\n");
			//}

			len = scnprintf(buf, BUFF_SIZE, "%d %d %d %d %d %d %d %d %d %d\n", r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9]);
			// Disable CNN
			iowrite32(0, tp->base_addr + XIL_CNN_START_OFFSET);
		}
		else {
			len = scnprintf(buf, BUFF_SIZE, "0\n");
		}
		...
		break;
	case 1:
		printk(KERN_INFO "cnn_read Succesfully read from CNN device 1.\n");
		len = scnprintf(buf, BUFF_SIZE, "0\n");
